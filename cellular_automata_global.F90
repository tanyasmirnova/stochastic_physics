...
use atmosphere_mod,    only: atmosphere_resolution, atmosphere_domain, &
                             atmosphere_scalar_field_halo, atmosphere_control_data, Atm
#endif
...
real(kind=kind_io4),  allocatable :: CAtmp(:,:,:)
...
logical              :: nca_plumes,new_smoothing
...
if (ca_smooth) then
if (new_smoothing) then
    allocate(CAtmp(Atm(1)%bd%isd:Atm(1)%bd%ied,Atm(1)%bd%jsd:Atm(1)%bd%jed,1))
    CAtmp(Atm(1)%bd%is:Atm(1)%bd%ie,Atm(1)%bd%js:Atm(1)%bd%je,1)=CA(1:nlon,1:nlat)
    call atmosphere_smooth_noise (CAtmp,100,2,0) !  do smoothing for RNDA
    CA(1:nlon,1:nlat)=CAtmp(Atm(1)%bd%is:Atm(1)%bd%je,Atm(1)%bd%js:Atm(1)%bd%je,1)
    deallocate(CAtmp)
else
do nn=1,100
...
do j=1,nlat
 do i=1,nlon
    CA1(i,j) = ((CA1(i,j) - Detmin(1))/(Detmax(1)-Detmin(1)))*2.0
 enddo
enddo
...
do j=1,nlat
 do i=1,nlon
    CA2(i,j) = ((CA2(i,j) - Detmin(2))/(Detmax(2)-Detmin(2)))*2.0
 enddo
enddo
...
do j=1,nlat
 do i=1,nlon
    CA3(i,j) = ((CA3(i,j) - Detmin(3))/(Detmax(3)-Detmin(3)))*2.0
 enddo
enddo
